...
def sampling(code_distance,p_list,rep):
    ...
    ############# detection_evemtを再構成 ################
    """
    re_detection_event_X = np.zeros((rep+1,code_distance-1,code_distance+1))
    for num in range(rep+1):
        for i in range(code_distance-1):
            for j in range(code_distance+1):
                if j == 0: #左端
    ...
    re_detection_event_Z = np.zeros((rep+1,code_distance+1,code_distance-1))
    for num in range(rep+1):
        for i in range(code_distance+1):
            for j in range(code_distance-1):
                if i == 0: #上端
...
if __name__ == "__main__":
    ...
    round_sur_list = [1]
    ### パラメータ ###
    round_rep_list = [20000,30000,40000,50000,60000,70000,80000]
    cd_rep_list = [3]
    trials = 20
    pro = 500
    ################
    ...
    for n in range(len(cd_rep_list)):
        for l in range(len(round_sur_list)):
            for i in range(len(cd_sur_list)):
                df_X = pd.DataFrame(data=c_X[n*len(round_sur_list)*len(cd_sur_list) + l*len(cd_sur_list): n*len(round_sur_list)*len(cd_sur_list) + l*len(cd_sur_list) + len(cd_sur_list)], columns=round_rep_list,index=cd_sur_list)
                df_X.to_csv('X,p='+str(p_list[0])+',d=('+str(d_s)+','+str(d_e)+','+str(d_d)+'),d(rep)='+str(cd_rep_list[n])+',eta='+str(eta)+',rep='+str(round_sur_list[l])+',trials='+str(trials*pro)+'.csv')
