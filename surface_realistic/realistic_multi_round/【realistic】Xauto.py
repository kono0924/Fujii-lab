...
if __name__ == "__main__":
    ...
    for i in range(len(cd_rep_list)):
        ...
        df_X.to_csv('Z error,p='+str(p)+',d2=('+str(d_s)+','+str(d_e)+','+str(d_d)+'),eta='+str(eta)+'round_rep='+str(round_rep)+',trials='+str(trials*pro)+'.csv')
        df_Z = pd.DataFrame(data=c_Z[i*len(cd_sur_list):(i+1)*len(cd_sur_list)], columns=div_list ,index=cd_sur_list)
        df_Z.to_csv('X error,p='+str(p)+',d2=('+str(d_s)+','+str(d_e)+','+str(d_d)+'),eta='+str(eta)+'round_rep='+str(round_rep)+',trials='+str(trials*pro)+'.csv')
        os.chdir('../') # ディレクトリ戻る
