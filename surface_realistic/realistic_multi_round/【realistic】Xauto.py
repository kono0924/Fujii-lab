...
def rotated_surface_code(code_distance,p_list,rep,div):
    ### エラーの履歴
    ...
    ### エラーシンドローム
    syndrome_in_X = np.zeros((rep*(div+1)+2, code_distance-1, code_distance-1)) #シンドローム測定の回数+最初の状態のシンドローム+最後の測定から計算したシンドローム
    syndrome_in_Z = np.zeros((rep*(div+1)+2, code_distance-1, code_distance-1)) #最初、最後のシンドローム測定、最後のデータの測定のみ
    syndrome_out_X = np.zeros((rep*(div+1)+2,2,int((code_distance-1)/2)))
    syndrome_out_Z = np.zeros((rep*(div+1)+2,2,int((code_distance-1)/2)))
    ...
    for num in range(rep):
        ...
        for num2 in range(div):
            ...
            for i in range(code_distance-1):
                for j in range(code_distance-1):
                    if (i+j)%2 == 0: ### Xシンドローム
                        ...
                        syndrome_in_X[num*(div+1)+num2+1][i][j] =  qubits_m_in[0][i][j] # Zを格納
                        qubits_m_in[0][i][j] = 0
                        qubits_m_in[1][i][j] = 0
            # 外側
            for i in range(int((code_distance-1)/2)):
                ...
                syndrome_out_X[num*(div+1)+num2+1][0][i] =  qubits_m_out_X[0][0][i] # 左
                ...
                syndrome_out_X[num*(div+1)+num2+1][1][i] =  qubits_m_out_X[0][1][i] # 右
        ...
        for i in range(code_distance-1):
            for j in range(code_distance-1):
                if (i+j)%2 == 0: ### Xシンドローム
                    ...
                    p_z_error(qubits_m_in,i,j,p_list[7])
                    p_x_error(qubits_m_in,i,j,p_list[6]) # 測定エラー
                    syndrome_in_X[num*(div+1)+div+1][i][j] =  qubits_m_in[0][i][j] # Zを格納
                    qubits_m_in[0][i][j] = 0
                    qubits_m_in[1][i][j] = 0
        # 外側
        for i in range(int((code_distance-1)/2)):
            ...
            p_z_error(qubits_m_out_X,0,i,p_list[7]) 
            p_x_error(qubits_m_out_X,0,i,p_list[6]) # 測定エラー
            syndrome_out_X[num*(div+1)+div+1][0][i] =  qubits_m_out_X[0][0][i] # 左
            qubits_m_out_X[0][0][i] = 0
            qubits_m_out_X[1][0][i] = 0
            H(qubits_m_out_X,1,i)
            p_x_error(qubits_m_out_X,1,i,p_list[6]) 
            p_z_error(qubits_m_out_X,1,i,p_list[7]) 
            p_x_error(qubits_m_out_X,1,i,p_list[6]) # 測定エラー
            syndrome_out_X[num*(div+1)+div+1][1][i] =  qubits_m_out_X[0][1][i] # 右
            qubits_m_out_X[0][1][i] = 0
            qubits_m_out_X[1][1][i] = 0
        ######################### 測定結果の格納 & 初期化 終わり#########################
    ...
    for i in range(code_distance-1):
        for j in range(code_distance-1):
            if (i+j)%2 == 0: 
                syndrome_in_X[rep*(div+1)+1][i][j] =  (qubits_d[0][i][j]+qubits_d[0][i][j+1]+qubits_d[0][i+1][j]+qubits_d[0][i+1][j+1]) % 2
    # 外側
    for i in range(int((code_distance-1)/2)):
        # 右
        syndrome_out_X[rep*(div+1)+1][1][i] = (qubits_d[0][2*i][code_distance-1]+qubits_d[0][2*i+1][code_distance-1]) % 2
        # 左
        syndrome_out_X[rep*(div+1)+1][0][i] = (qubits_d[0][2*i+1][0]+qubits_d[0][2*i+2][0]) % 2
    ### データ量子ビットをresult_dataに移す
    ...
    ############# detection eventの計算 ###############
    detection_event_in_X = np.zeros((rep*(div+1)+1, code_distance-1, code_distance-1))
    detection_event_in_Z = np.zeros((rep+1, code_distance-1, code_distance-1))
    detection_event_out_X = np.zeros((rep*(div+1)+1, 2, int((code_distance-1)/2)))
    detection_event_out_Z = np.zeros((rep+1, 2, int((code_distance-1)/2)))

    ### Xシンドローム
    for num in range(rep*(div+1)+1):
        ### 内側
        for i in range(code_distance-1):
            for j in range(code_distance-1):
                if (i+j) % 2 == 0:
    ...
    for num in range(rep+1):
        for i in range(-1,code_distance-1):
            for j in range(code_distance-2):
                if num == 0:
                    if (i+j) % 2 == 1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log((p_list[0]+1*div*p_list[6])))
                    elif (i+j) % 2 == 0:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log((p_list[0]+3*div*p_list[6])))
                elif num == rep:
                    if (i+j) % 2 == 1 and i == -1:
                    ...
                    if (i+j) % 2 == 1 and i == -1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[8]+p_list[2]+p_list[6]+0*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 1 and i == code_distance-2:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[8]+p_list[2]+p_list[6]+1*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[8]+p_list[2]+2*p_list[6]+1*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 0 and i == -1:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[8]+p_list[2]+p_list[6]+1*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 0 and i == code_distance-2:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[8]+p_list[2]+p_list[6]+2*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 0:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[8]+p_list[2]+2*p_list[6]+3*(div+1)*p_list[6]))
                else:
                    if (i+j) % 2 == 1 and i == -1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[0]+p_list[2]+p_list[6]+0*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 1 and i == code_distance-2:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[0]+p_list[2]+p_list[6]+1*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[0]+p_list[2]+2*p_list[6]+1*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 0 and i == -1:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[0]+p_list[2]+p_list[6]+1*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 0 and i == code_distance-2:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[0]+p_list[2]+p_list[6]+2*(div+1)*p_list[6]))
                    elif (i+j) % 2 == 0:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[0]+p_list[2]+2*p_list[6]+3*(div+1)*p_list[6]))
            ...
            if num == 0:
                if i % 2 == 0:
                    if i == code_distance-1:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((p_list[0]+div*2*p_list[6])))
                    elif i == 0:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[0]+div*1*p_list[6])))
                    else:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[0]+div*3*p_list[6])))
                if i % 2 == 1:
                    if i == -1:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((p_list[0]+div*1*p_list[6])))
                    if i == code_distance-2:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[0]+div*2*p_list[6])))
                    else:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[0]+div*3*p_list[6])))
            elif num == rep:
                if i % 2 == 0:
                    if i == code_distance-1: # 半円 ()外は伝播によるもの
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((p_list[2]+(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                    elif i == 0: # 下四角
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[2]+3*(div+1)*p_list[6])+1*(div+1)*p_list[6]))
                    else: # 中四角
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[2]+4*(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                if i % 2 == 1:
                    if i == -1: # 半円
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((p_list[2]+(div+1)*p_list[6])+1*(div+1)*p_list[6]))
                    if i == code_distance-2: # 下四角
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[2]+3*(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                    else: # 中四角
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[2]+4*(div+1)*p_list[6])+3*(div+1)*p_list[6]))
            elif num == rep-1:
                if i % 2 == 0:
                    if i == code_distance-1:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((p_list[8]+p_list[2]+(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                    elif i == 0:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[8]+2*p_list[2]+3*(div+1)*p_list[6])+1*(div+1)*p_list[6]))
                    else:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[8]+2*p_list[2]+4*(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                if i % 2 == 1:
                    if i == -1:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((p_list[8]+p_list[2]+(div+1)*p_list[6])+1*(div+1)*p_list[6]))
                    if i == code_distance-2:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[8]+2*p_list[2]+3*(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                    else:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[8]+2*p_list[2]+4*(div+1)*p_list[6])+3*(div+1)*p_list[6]))
            else:        ###### ここから修正(測定と初期化を足すだけ)
                if i % 2 == 0:
                    if i == code_distance-1:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((p_list[0]+p_list[2]+(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                    elif i == 0:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[0]+2*p_list[2]+3*(div+1)*p_list[6])+1*(div+1)*p_list[6]))
                    else:
                        gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[0]+2*p_list[2]+4*(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                if i % 2 == 1:
                    if i == -1:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((p_list[0]+p_list[2]+(div+1)*p_list[6])+1*(div+1)*p_list[6]))
                    if i == code_distance-2:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[0]+2*p_list[2]+3*(div+1)*p_list[6])+2*(div+1)*p_list[6]))
                    else:
                        gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[0]+2*p_list[2]+4*(div+1)*p_list[6])+3*(div+1)*p_list[6]))
...
if __name__ == "__main__":
    ...
    for _ in range(pro):
        ...
        process = multiprocessing.Process(target=count, args=(trials,cd_sur_list,p,eta,round_rep,cd_rep_list,rep,div_list,result_list,))
        # プロセス開始
        process.start()
        # プロセスのリストに追加
    ...
    for i in range(len(cd_rep_list)):
        if os.path.exists('multiX,d1='+str(cd_rep_list[i])+',p='+str(p)+',eta='+str(eta))==False:
            os.mkdir('multiX,d1='+str(cd_rep_list[i])+',p='+str(p)+',eta='+str(eta))

    for i in range(len(cd_rep_list)):
        os.chdir('multiX,d1='+str(cd_rep_list[i])+',p='+str(p)+',eta='+str(eta))
