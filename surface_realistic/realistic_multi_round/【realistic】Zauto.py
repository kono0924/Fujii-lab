...
def rotated_surface_code(code_distance,p_list,rep,div):
    ...
    ### エラーシンドローム
    syndrome_in_X = np.zeros((rep+2, code_distance-1, code_distance-1)) #最初、最後のシンドローム測定、最後のデータの測定のみ   
    syndrome_in_Z = np.zeros((rep*(div+1)+2, code_distance-1, code_distance-1)) #シンドローム測定の回数+最初の状態のシンドローム+最後の測定から計算したシンドローム
    ...
    for num in range(rep):
        ...
        for num2 in range(div):
            ...
            for i in range(int((code_distance-1)/2)):
                p_x_error(qubits_m_in,i,j,p_list[6]) # 測定エラー
                syndrome_out_Z[num*(div+1)+num2+1][0][i] =  qubits_m_out_Z[0][0][i] # 上
    ...
    for i in range(code_distance-1):
        for j in range(code_distance-1):
            if (i+j)%2 == 1: 
                syndrome_in_Z[rep*(div+1)+1][i][j] =  (qubits_d[0][i][j]+qubits_d[0][i][j+1]+qubits_d[0][i+1][j]+qubits_d[0][i+1][j+1]) % 2
    # 外側
    for i in range(int((code_distance-1)/2)):
        # 上
        syndrome_out_Z[rep*(div+1)+1][0][i] = (qubits_d[0][0][2*i]+qubits_d[0][0][2*i+1]) % 2
        # 下
        syndrome_out_Z[rep*(div+1)+1][1][i] = (qubits_d[0][code_distance-1][2*i+1]+qubits_d[0][code_distance-1][2*i+2]) % 2
    ...
    detection_event_in_Z = np.zeros((rep*(div+1)+1, code_distance-1, code_distance-1))
    detection_event_out_X = np.zeros((rep+1, 2, int((code_distance-1)/2)))
    detection_event_out_Z = np.zeros((rep*(div+1)+1, 2, int((code_distance-1)/2)))
...
def sampling(code_distance,p_list,rep,div):
        ...
        for i in range(-1,code_distance-1):
            for j in range(code_distance-2):
                if num == 0:
                    ...
                    if (i+j) % 2 == 1 and i == -1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[0]+p_list[2]))
                    elif (i+j) % 2 == 1 and i == code_distance-2:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[0]+p_list[2]))
                    elif (i+j) % 2 == 1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[0]+p_list[2]))
                    elif (i+j) % 2 == 0 and i == -1:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[0]+p_list[2]))
                    elif (i+j) % 2 == 0 and i == code_distance-2:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[0]+p_list[2]))
                    elif (i+j) % 2 == 0:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[0]+p_list[2]))
    ...
    for num in range(rep*(div+1)+1):
        ...
        for i in range(-1,code_distance):
            if num == 0:
                if i % 2 == 0 and i == code_distance-1:
                ...
                elif i % 2 == 1 and i == code_distance-2:
                ...
                if i % 2 == 0 and i == code_distance-1:
                    ...
                    gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[0]+2*p_list[2])))
                elif i % 2 == 0:
                    gp_Z.add_edge('external_Z',(num,i,code_distance-2),weight=-math.log((2*p_list[0]+2*p_list[2])))
                elif i % 2 == 1 and i == -1:
                    gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((p_list[0]+p_list[2])))
                elif i % 2 == 1 and i == code_distance-2:
                    gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[0]+2*p_list[2])))
                elif i % 2 == 1:
                    gp_Z.add_edge('external_Z',(num,i,0),weight=-math.log((2*p_list[0]+2*p_list[2])))
        ...
        for i in range(code_distance-2):
            for j in range(-1,code_distance-1):
                if num == 0:
                    ...
                    if (i+j) % 2 == 0 and j==-1:
                        gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log((div+1)*(p_list[1]+p_list[3]+1*p_list[5])))
                    elif (i+j) % 2 == 0 and j==code_distance-2:
                        gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log((div+1)*(p_list[1]+p_list[3]+0*p_list[5])))
                    elif (i+j) % 2 == 0:
                        gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log((div+1)*(p_list[1]+2*p_list[3]+1*p_list[5])))
                    elif (i+j) % 2 == 1 and j==-1:
                        gp_X.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log((div+1)*(p_list[1]+p_list[3]+1*p_list[5])))
                    elif (i+j) % 2 == 1 and j==code_distance-2:
                        gp_X.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log((div+1)*(p_list[1]+2*p_list[3]+2*p_list[5])))
                    elif (i+j) % 2 == 1:
                        gp_X.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log((div+1)*(p_list[1]+p_list[3]+3*p_list[5])))
        ...
        for j in range(-1,code_distance):
            if num == 0:
                if j % 2 == 0 and j == 0:
                    ...
                    gp_X.add_edge('external_X',(num,0,j),weight=-math.log((div+1)*(p_list[1]+4*p_list[3]+0*p_list[5])))
                elif j % 2 == 0:
                    gp_X.add_edge('external_X',(num,0,j),weight=-math.log((div+1)*(2*p_list[1]+p_list[3]+2*p_list[5])))
                elif j % 2 == 1 and j == -1:
                    gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log((div+1)*(p_list[1]+4*p_list[3]+1*p_list[5])))
                elif j % 2 == 1 and j == code_distance-2:
                    gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log((div+1)*(2*p_list[1]+3*p_list[3]+3*p_list[5])))
                elif j % 2 == 1:
                    gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log((div+1)*(2*p_list[1]+p_list[3]+2*p_list[5])))
            elif num == rep:
                continue
                if j % 2 == 0 and j == 0:
...
def count(trials,cd_sur_list,p,eta,round_rep,cd_rep_list,rep,div_list,result_list):
    count_X = np.zeros((len(cd_rep_list)*len(div_list),len(cd_sur_list)))
    count_Z = np.zeros((len(cd_rep_list)*len(div_list),len(cd_sur_list)))
