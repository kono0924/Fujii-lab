...
def rotated_surface_code(code_distance,p_list,rep):
    ...
    for i in range(code_distance):
        for j in range(code_distance):
            p_x_error(qubits_d,i,j,p_list[11])
    ### 測定前のXエラー
    for i in range(code_distance):
        for j in range(code_distance):
        ...
        for i in range(-1,code_distance-1):
            for j in range(code_distance-2):
                if num == 0:
                    ...
                    if (i+j) % 2 == 1 and i == -1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[10]+p_list[6]+p_list[2]+0*p_list[8]+p_list[11])) # 先頭は最後の測定エラー、末尾はアダマール
                    elif (i+j) % 2 == 1 and i == code_distance-2:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[10]+p_list[6]+p_list[2]+1*p_list[8]+p_list[11]))
                    elif (i+j) % 2 == 1:
                        gp_Z.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[10]+2*p_list[6]+p_list[2]+1*p_list[8]+p_list[11]))
                    elif (i+j) % 2 == 0 and i == -1:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[10]+p_list[6]+p_list[2]+1*p_list[8]+p_list[11]))
                    elif (i+j) % 2 == 0 and i == code_distance-2:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[10]+p_list[6]+p_list[2]+2*p_list[8]+p_list[11]))
                    elif (i+j) % 2 == 0:
                        gp_Z.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[10]+2*p_list[6]+p_list[2]+3*p_list[8]+p_list[11]))
                else:
                    if (i+j) % 2 == 1 and i == -1:
...
if __name__ == "__main__":
    ...
    ### パラメータ ###
    cd_rep_list = [3,5,7]
    p_list = [0.001,0.002,0.003,0.004,0.005,0.006,0.007,0.008,0.009,0.01]
    ...
    for i in range(len(cd_rep_list)):
        ...
        for k in range(100):
            if os.path.exists('X error,N='+str(1)+',d2=('+str(d_s)+','+str(d_e)+','+str(d_d)+'),eta='+str(eta)+',trials='+str(trials*pro)+',ver'+str(k)+'.csv')==True:
                ...
                df_Z.to_csv('X error(m),d2=('+str(d_s)+','+str(d_e)+','+str(d_d)+'),eta='+str(eta)+',trials='+str(trials*pro)+',ver'+str(k)+'.csv')
                break
        os.chdir('../') # ディレクトリ戻る
