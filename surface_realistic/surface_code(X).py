...
def rotated_surface_code(code_distance,p_L,p_g,p_syn,p_m):
        ...
        for i in range(code_distance-1):
            for j in range(code_distance-1):
                ...
                if (i+j)%2 == 0: 
                    ...
                    p_z_error(qubits_d,i,j,p_g)
                    p_z_error(qubits_m_in,i,j,p_syn)
                    CNOT(qubits_m_in,i,j,qubits_d,i,j+1)
                    p_z_error(qubits_d,i,j+1,p_g)
                    p_z_error(qubits_m_in,i,j,p_syn)
                    CNOT(qubits_m_in,i,j,qubits_d,i+1,j)
                    p_z_error(qubits_d,i+1,j,p_g)
                    p_z_error(qubits_m_in,i,j,p_syn)
                    CNOT(qubits_m_in,i,j,qubits_d,i+1,j+1)
                    p_z_error(qubits_d,i+1,j+1,p_g)
                    p_z_error(qubits_m_in,i,j,p_syn)
                # 外側
                if j == 0:
                    if i % 2 == 1:
                        ...
                        p_z_error(qubits_d,i,j,p_g)
                        p_z_error(qubits_m_out_X,0,int((i-1)/2),p_syn)
                ...
                if j == code_distance-2:
                    if i % 2 == 0:
                        ...
                        p_z_error(qubits_d,i,code_distance-1,p_g)
                        p_z_error(qubits_m_out_X,1,int(i/2),p_syn)
                        CNOT(qubits_m_out_X,1,int(i/2),qubits_d,i+1,code_distance-1)
                        p_z_error(qubits_d,i+1,code_distance-1,p_g)
                        p_z_error(qubits_m_out_X,1,int(i/2),p_syn)        
...
def sampling(code_distance,p_L,p_g,p_syn,p_m):

    ############# 読み込み ################

    detection_event_in, detection_event_out, result_data = rotated_surface_code(code_distance,p_L,p_g,p_syn,p_m)
        ...
        for num in range(code_distance):
            for i in range(code_distance-1):
                for j in range(-1,code_distance):
                    if (i+j) % 2 == 0:
                        gp.add_edge((num,i,j),(num+1,i,j),weight=-math.log(p_syn))
    ### 横
    for num in range(code_distance+1):
        for i in range(code_distance-2):
            for j in range(-1,code_distance):
                if (i+j) % 2 == 0:
                    gp.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_g))
                if (i+j) % 2 == 1:
                    gp.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_g))
    ### 斜め
    for num in range(code_distance):
        ### 内側
        for i in range(code_distance-2):
            for j in range(code_distance-2):
                if (i+j) % 2 == 0:
                    gp.add_edge((num+1,i,j),(num,i+1,j+1),weight=-math.log(p_g))
                if (i+j) % 2 == 1:
                    gp.add_edge((num+1,i+1,j),(num,i,j+1),weight=-math.log(p_g))
        ### 外側
        for i in range(code_distance-2):
            if i % 2 == 0:
                gp.add_edge((num,i,code_distance-1),(num+1,i+1,code_distance-2),weight=-math.log(p_g))
                gp.add_edge((num,i+1,-1),(num+1,i,0),weight=-math.log(p_g))
            if i % 2 == 1:
                gp.add_edge((num,i+1,code_distance-1),(num+1,i,code_distance-2),weight=-math.log(p_g))
                gp.add_edge((num,i,-1),(num+1,i+1,0),weight=-math.log(p_g))

    ### 外点
    for num in range(code_distance+1):
        for j in range(-1,code_distance):     
            if j % 2 == 0:
                gp.add_edge('external',(num,0,j),weight=-math.log(p_L))
            if j % 2 == 1:
                gp.add_edge('external',(num,code_distance-2,j),weight=-math.log(p_L))
    ...
    for num in range(code_distance+1):
        for i in range(int((code_distance-1)/2)):
            if detection_event_out[num,1,i] == 1:
                edge_of_decoder_graph.append((num,2*i,code_distance-1))
            if detection_event_out[num,0,i] == 1:
    ...
    for i in range(len(edge_of_decoder_graph)):
        for j in range(i):
            ...
            mwpm_gp.add_edge(i,j,weight = 1000000000000 - shortest_path_weight)
...
def count(trials,code_distance,p_div,result_list):
    count = np.zeros((len(code_distance),len(p_div[0])))
