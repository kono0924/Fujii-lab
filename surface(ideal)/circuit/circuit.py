...
def rotated_surface_code(code_distance,p_list,round_sur):
    ...
    for num in range(round_sur):
        ...
        for i in range(code_distance-1):
            for j in range(code_distance-1):    
                ...
                if (i+j)%2 == 1: 
                    ...
                    #p_z_error(qubits_m_in,i,j,p_list[1])
                ...
                if i == 0:
                    if j % 2 == 0:
                        ...
                        #p_z_error(qubits_m_out_Z,0,int(i/2),p_list[1])
                ...
                if i == code_distance-2:
                    if j % 2 == 1:
                        ...
                        #p_z_error(qubits_d,code_distance-1,code_distance-1,p_list[1])
                        #p_x_error(qubits_m_out_Z,1,int((i-1)/2),p_list[6])
                        #p_z_error(qubits_m_out_Z,1,int((i-1)/2),p_list[1])
            ...
            for j in range(code_distance-1):
                ...
                if j == 0:
                    if i % 2 == 1:
                        ...
                        #p_x_error(qubits_d,i,j,p_list[2])
                        p_z_error(qubits_d,i,j,p_list[1])
                        p_z_error(qubits_m_out_X,0,int((i-1)/2),p_list[1])
        ...
        for i in range(code_distance-1):
            for j in range(code_distance-1):
                if (i+j)%2 == 0: ### Xシンドローム
                    p_x_error(qubits_m_in,i,j,p_list[1]) #初期化反転
                    H(qubits_m_in,i,j)
        ...
        # 外側
        for i in range(int((code_distance-1)/2)):
            p_x_error(qubits_m_out_X,0,i,p_list[1]) #初期化反転
            ...
            p_x_error(qubits_m_out_X,1,i,p_list[1]) #初期化反転
    ...
    for num in range(code_distance):
        ...
        for i in range(code_distance-1):
            for j in range(-1,code_distance):
                if (i+j) % 2 == 0:
                    gp_X.add_edge((num,i,j),(num+1,i,j),weight=-math.log(5*p_list[1]))
        ### 外側
        for i in range(code_distance-1):
            if i % 2 == 0:
                gp_X.add_edge((num,i,code_distance-1),(num+1,i,code_distance-1),weight=-math.log(3*p_list[1]))
            if i % 2 == 1:
                gp_X.add_edge((num,i,-1),(num+1,i,-1),weight=-math.log(3*p_list[1]))
        ...
        for i in range(code_distance-2):
            for j in range(-1,code_distance-1):
                if num == 0:
                    ...
                    if (i+j) % 2 == 0:
                        gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(2*p_list[1]))
                    if (i+j) % 2 == 1:
                        gp_X.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(2*p_list[1]))
                else:
                    if (i+j) % 2 == 0:
                        gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[1]))
                    if (i+j) % 2 == 1:
    ...
    for num in range(code_distance):
        ...
        for i in range(code_distance-2):
            for j in range(code_distance-2):
                ...
                if (i+j) % 2 == 1:
                    gp_X.add_edge((num+1,i+1,j),(num,i,j+1),weight=-math.log(p_list[1]))
        ### 外側
        for i in range(code_distance-2):
            if i % 2 == 0:
                gp_X.add_edge((num,i,code_distance-1),(num+1,i+1,code_distance-2),weight=-math.log(p_list[1]))
                gp_X.add_edge((num,i+1,-1),(num+1,i,0),weight=-math.log(p_list[1]))
            if i % 2 == 1:
                gp_X.add_edge((num,i+1,code_distance-1),(num+1,i,code_distance-2),weight=-math.log(p_list[1]))
    ...
    for num in range(round_sur+1):
        for j in range(-1,code_distance):
            if num == 0:
                ...
                if j % 2 == 0 and j != code_distance-1:
                    gp_X.add_edge('external_X',(num,0,j),weight=-math.log(4*p_list[1]))
                if j % 2 == 0 and j == code_distance-1:
                    gp_X.add_edge('external_X',(num,0,j),weight=-math.log(2*p_list[1]))
                if j % 2 == 1 and j != -1:
                    gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log(4*p_list[1]))
                if j % 2 == 1 and j == -1:
                    gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log(2*p_list[1]))
            else:
                if j % 2 == 0 and j != code_distance-1:
                    gp_X.add_edge('external_X',(num,0,j),weight=-math.log(2*p_list[1]))
                if j % 2 == 0 and j == code_distance-1:
                    gp_X.add_edge('external_X',(num,0,j),weight=-math.log(1*p_list[1]))
                if j % 2 == 1 and j != -1:
                    gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log(2*p_list[1]))
                if j % 2 == 1 and j == -1:
                    gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log(1*p_list[1]))
    ...
    for _ in range(trials):
        for i in range(len(cd_sur_list)):
            for j in range(len(p_list)):
                result_data_Z, modefied_result_Z, judge_X, result_data_X, modefied_result_X, judge_Z  = sampling(cd_sur_list[i],p_matrix(p_list[j],eta,100//cd_sur_list[i],cd_rep),cd_sur_list[i])
                if judge_X == 1:
                    count_X[i,j] += 1
                #print("result_Z\n", result_data_Z)
