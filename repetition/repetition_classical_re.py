...
def repetition(code_distance,rep,p,eta):
    ...
    for num in range(rep):
        ...
        for i in range(code_distance-1):
            ...
            bitflip_error(qubit,2*i+1,p,eta) # 測定エラー
...
if __name__ == "__main__":
    ...
    for k in range(len(p_list)):
        ...
        df.to_csv('p='+str(p_list[k])+' ,eta='+str(eta)+', d=('+str(code_distance_list[0])+','+str(code_distance_list[-1])+',2), round=('+str(rep_sta)+','+str(rep_fin)+','+str(rep_div)+') , # of trials='+str(trials*pro)+'.csv')
