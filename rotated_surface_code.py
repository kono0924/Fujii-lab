...
def rotated_surface_code(code_distance,p):
    ...
    detection_event_out_X = np.zeros((code_distance+1, 4, int((code_distance-1)/2)))
    detection_event_out_Z = np.zeros((code_distance+1, 4, int((code_distance-1)/2)))
...
def sampling(code_distance,p):

    ############# 読み込み ################

    detection_event_in, detection_event_out, result_data = rotated_surface_code(code_distance,p)
    ...
    for num in range(code_distance+1):
        for j in range(-1,code_distance):     
            ...
            if j % 2 == 1:
                gp.add_edge('external',(num,code_distance-2,i),weight=-math.log(p))
    ...
    for cd in np.arange(3,code_distance +1,2):
        ...
        for p in p_div:
            ...
            for _ in range(trial):
                result_data, result, judge = sampling(cd,p/100)
                #print("\nafter= \n",result, "\n", judge)
                if judge == 1:
                    count[int((cd-3)/2),num] += 1
            num += 1
    for cd in np.arange(3,code_distance +1,2):
        num =0
        for p in p_div:
...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    for cd in np.arange(3,code_distance +1,2):
        ax.plot(p_div,c[int((cd-3)/2)]*100,marker='v',label="d ="+str(cd))
    ...
    ax.set_title("pm=" + str(0) + "%", fontsize=14)
    plt.legend()
    plt.savefig("pm=" + str(0) + "%.pdf")
