...
def sampling(code_distance,p,p_m):
    ...
    for i in range(len(edge_of_decoder_graph)):
        for j in range(i):
            ...
            mwpm_gp.add_edge(i,j,weight = 1000000000 - shortest_path_weight)
    ...
    for match_pair in mwpm_res:
        ...
        #print(nx.dijkstra_path(gp,edge_of_decoder_graph[match_pair[0]],edge_of_decoder_graph[match_pair[1]]),
    ...
    for num in range(code_distance):
        ...
        for i in range(code_distance-1):
            for j in range(-1,code_distance):
                if (i+j) % 2 == 1 and re_detection_event[num][i][j] == 0:
                    re_detection_event[num][i][j] = 0.5
