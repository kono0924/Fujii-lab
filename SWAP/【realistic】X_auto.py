...
def rotated_surface_code(code_distance,p_list,round_sur):
    ...
    for num in range(round_sur):
        ...
        for i in range(code_distance-1):
            for j in range(code_distance-1):
                if (i+j)%2 == 0: 
                    ...
                    p_x_error(qubits_m_in,i,j,p_list[6])
                    p_z_error(qubits_m_in,i,j,p_list[2])
                    CNOT(qubits_m_in,i,j,qubits_d,i,j+1)
                    p_x_error(qubits_d,i,j+1,p_list[0])
                    p_z_error(qubits_d,i,j+1,p_list[1])
                    p_x_error(qubits_m_in,i,j,p_list[6])
                    p_z_error(qubits_m_in,i,j,p_list[2])
                    CNOT(qubits_m_in,i,j,qubits_d,i+1,j)
                    p_x_error(qubits_d,i+1,j,p_list[0])
                    p_z_error(qubits_d,i+1,j,p_list[1])
                    p_x_error(qubits_m_in,i,j,p_list[6])
                    p_z_error(qubits_m_in,i,j,p_list[2])
                    CNOT(qubits_m_in,i,j,qubits_d,i+1,j+1)
                    p_x_error(qubits_d,i+1,j+1,p_list[0])
                    p_z_error(qubits_d,i+1,j+1,p_list[1])
                    p_x_error(qubits_m_in,i,j,p_list[6])
                    p_z_error(qubits_m_in,i,j,p_list[2])
        # 外側
        for i in range(code_distance-1):
            for j in range(code_distance-1):
                if j == 0:
                    if i % 2 == 1:
                        ...
                        p_z_error(qubits_m_out_X,0,int((i-1)/2),p_list[2])
                ...
                if j == code_distance-2:
                    if i % 2 == 0:
                        ...
                        p_x_error(qubits_d,i,code_distance-1,p_list[0])
                        p_z_error(qubits_d,i,code_distance-1,p_list[1])
                        p_x_error(qubits_m_out_X,1,int(i/2),p_list[6])
                        p_z_error(qubits_m_out_X,1,int(i/2),p_list[2])
                        CNOT(qubits_m_out_X,1,int(i/2),qubits_d,i+1,code_distance-1)
                        p_x_error(qubits_d,i+1,code_distance-1,p_list[0])
            ...
            for i in range(code_distance-1):
                for j in range(code_distance-1):    
                    if (i+j)%2 == 1: 
                        ...
                        p_x_error(qubits_d,i,j,p_list[3])
                        p_z_error(qubits_d,i,j,p_list[4])
                        p_x_error(qubits_m_in,i,j,p_list[5])
                        p_z_error(qubits_m_in,i,j,p_list[7])
                        CNOT(qubits_d,i,j+1,qubits_m_in,i,j)
                        p_x_error(qubits_d,i,j+1,p_list[3])
                        p_z_error(qubits_d,i,j+1,p_list[4])
                        p_x_error(qubits_m_in,i,j,p_list[5])
                        p_z_error(qubits_m_in,i,j,p_list[7])
                        CNOT(qubits_d,i+1,j,qubits_m_in,i,j)
                        p_x_error(qubits_d,i+1,j,p_list[3])
                        p_z_error(qubits_d,i+1,j,p_list[4])
                        p_x_error(qubits_m_in,i,j,p_list[5])
                        p_z_error(qubits_m_in,i,j,p_list[7])
                        CNOT(qubits_d,i+1,j+1,qubits_m_in,i,j)
                        p_x_error(qubits_d,i+1,j+1,p_list[3])
                        p_z_error(qubits_d,i+1,j+1,p_list[4])
                        p_x_error(qubits_m_in,i,j,p_list[5])
                        p_z_error(qubits_m_in,i,j,p_list[7])
            # 外側
            for i in range(code_distance-1):
                for j in range(code_distance-1):  
                    if i == 0:
                        if j % 2 == 0:
                            ...
                            p_x_error(qubits_d,i,j,p_list[3])
                            p_z_error(qubits_d,i,j,p_list[4])
                            p_x_error(qubits_m_out_Z,0,int(i/2),p_list[5])
                            p_z_error(qubits_m_out_Z,0,int(i/2),p_list[7])
                    ...
                    if i == code_distance-2:
                        if j % 2 == 1:
                            ...
                            p_x_error(qubits_d,code_distance-1,code_distance-1,p_list[3])
                            p_z_error(qubits_d,code_distance-1,code_distance-1,p_list[4])
                            p_x_error(qubits_m_out_Z,1,int((i-1)/2),p_list[5])
                            p_z_error(qubits_m_out_Z,1,int((i-1)/2),p_list[7])
        ...
        for i in range(int((code_distance-1)/2)):
            ...
            p_x_error(qubits_m_out_X,0,i,p_list[6]) #測定結果反転 (本番では消す)
            p_z_error(qubits_m_out_X,0,i,p_list[7]) #測定結果反転 (本番では消す)
            syndrome_out_X[num+1][0][i] =  qubits_m_out_X[0][0][i] # 左
            qubits_m_out_X[0][0][i] = 0
            qubits_m_out_X[1][0][i] = 0
            H(qubits_m_out_X,1,i)
            p_x_error(qubits_m_out_X,1,i,p_list[6]) #測定結果反転 (本番では消す)
            p_z_error(qubits_m_out_X,1,i,p_list[7]) #測定結果反転 (本番では消す)
    ...
    for num in range(round_sur):
        ...
        for i in range(code_distance-1):
            for j in range(-1,code_distance):
                if (i+j) % 2 == 0:
                    gp_X.add_edge((num,i,j),(num+1,i,j),weight=-math.log(7*p_list[6]+p_list[7]+4*p_list[2]))
        ### 外側
        for i in range(code_distance-1):
            if i % 2 == 0:
                gp_X.add_edge((num,i,code_distance-1),(num+1,i,code_distance-1),weight=-math.log(7*p_list[6]+p_list[7]+2*p_list[2]))
            if i % 2 == 1:
                gp_X.add_edge((num,i,-1),(num+1,i,-1),weight=-math.log(7*p_list[6]+p_list[7]+2*p_list[2]))
        ...
        for i in range(code_distance-2):
            for j in range(-1,code_distance-1):
                ...
                if num == round_sur:
                    if (i+j) % 2 == 0:
                        if j == -1:
                            gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[1]+p_list[4]+2*(p_list[6]+p_list[7])+p_list[7])) # Xシンドローム測定とZの分とアダマール、測定
                        if j == code_distance-2:
                            gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[1]+p_list[4]+2*(p_list[6]+p_list[7]))) # Xシンドローム測定とZの分とアダマール、測定
                        else:
                            gp_X.add_edge((num,i,j),(num,i+1,j+1),weight=-math.log(p_list[1]+2*p_list[4]+2*(p_list[6]+p_list[7])+p_list[7])) # Xシンドローム測定とZの分とアダマール、測定
                    if (i+j) % 2 == 1:
                        if j == -1:
                            gp_X.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[1]+p_list[4]+2*(p_list[6]+p_list[7])+p_list[7]))
                        if j == code_distance-2:
                            gp_X.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[1]+p_list[4]+2*(p_list[6]+p_list[7])+2*p_list[7]))
                        else:
                            gp_X.add_edge((num,i+1,j),(num,i,j+1),weight=-math.log(p_list[1]+2*p_list[4]+2*(p_list[6]+p_list[7])+3*p_list[7]))
                else:
                    if (i+j) % 2 == 0:
    ...
    for num in range(round_sur):
        ...
        for i in range(-1,code_distance-1):
            for j in range(code_distance-2):
                if (i+j) % 2 == 0:
                    gp_X.add_edge((num+1,i,j),(num,i+1,j+1),weight=-math.log(p_list[1]))
                if (i+j) % 2 == 1:
                    gp_X.add_edge((num,i+1,j),(num+1,i,j+1),weight=-math.log(p_list[1]))
        ### 外側
        for i in range(code_distance-2):
            if i % 2 == 0:
                gp_X.add_edge((num+1,i+1,code_distance-2),(num,i,code_distance-1),weight=-math.log(p_list[1]))
                gp_X.add_edge((num,i,),(num+1,i+1,-1),weight=-math.log(p_list[1]))
            if i % 2 == 1:
                gp_X.add_edge((num,i,code_distance-2),(num+1,i+1,code_distance-1),weight=-math.log(p_list[1]))
                gp_X.add_edge((num,i,-1),(num+1,i+1,0),weight=-math.log(p_list[1]))
    ...
    for num in range(round_sur+1):
        for j in range(-1,code_distance):
            if num == round_sur:
                if j % 2 == 0:
                    if j == 0:
                        gp_X.add_edge('external_X',(num,0,j),weight=-math.log(p_list[1]+3*p_list[4]))
                    if j == code_distance-1:
                        gp_X.add_edge('external_X',(num,0,j),weight=-math.log(p_list[1]+p_list[4]))
                    else:
                        gp_X.add_edge('external_X',(num,0,j),weight=-math.log(2*p_list[1]+4*p_list[4]))
                if j % 2 == 1:
                    if i == -1:
                        gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log(p_list[1]+p_list[4]))
                    if i == code_distance-2:
                        gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log(p_list[1]+3*p_list[4]))
                    else:
                        gp_X.add_edge('external_X',(num,code_distance-2,j),weight=-math.log(2*p_list[1]+4*p_list[4]))
            else:
                if j % 2 == 0:
                    if j == code_distance-1:
...
# 論理CNOTゲート
def pgx_x(p,eta,cd_rep):
    P = p/(eta+1)
    return 1/2 * (1-(1-2*P)**(3*cd_rep-2))

# 論理CNOTゲート
def pgz_z(p,eta,cd_rep):
    P = p*(2*eta+1)/(2*eta+2)
    return 1/2 * (1-(1-2*P)**(3*cd_rep-2))

def p_matrix(p,eta,round_rep,cd_rep):
    matrix = []
    matrix.append(pgx_x(p,eta,cd_rep)) #pgx_x
    matrix.append(2*p*(2*eta+1)/(2*eta+2)) # pgx_z  
    matrix.append(cd_rep*p*(2*eta+1)/(2*eta+2)) # pgx_syn
    matrix.append(2*p/(eta+1)) #pgz_x
    matrix.append(pgz_z(p,eta,cd_rep)) # pgz_z
    matrix.append(cd_rep*p/(eta+1)) # pgz_syn
    ...
    for i in range(len(cd_rep_list)):
        for j in range(len(round_rep_list)):
            for k in range(len(round_sur_list)):
                ...
                df_X.to_csv('X,p=('+str(p_s)+','+str(p_e)+','+str(p_d)+'),d=('+str(d_s)+','+str(d_e)+','+str(d_d)+'),d(rep)='+str(cd_rep_list[i])+',eta='+str(eta)+',round_sur='+str(round_sur_list[k])+',trials='+str(trials*pro)+'.csv')
